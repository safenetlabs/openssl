...
func (ctx *hmacCtx) HmacUpdate(input []byte) error {
	C.HMAC_Update(ctx.hmacContext(), (*C.uchar)(&input[0]), C.size_t(len(input)))
	// TODO: signature mismatch between OSX and Linux. OSX returns void
	//	if res != 1 {
...
func (ctx *hmacCtx) HmacFinal() ([]byte, error) {
	...
	C.HMAC_Final(ctx.hmacContext(), (*C.uchar)(&outbuf[0]), &outlen)
	// TODO: signature mismatch between OSX and Linux. OSX returns void
	//	if res != 1 {
